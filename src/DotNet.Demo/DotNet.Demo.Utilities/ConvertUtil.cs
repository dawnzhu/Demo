...
namespace DotNet.Demo.Utilities
{
    public static class ConvertUtil
    {
        ...
        /// <param name="t"></param>
        /// <returns></returns>
        public static IList<EnumInfo> ToList(this Type t)
        {
            return ToList(t, null, null);
        }
        ...
        /// <returns></returns>
        public static IList<EnumInfo> ToList(this Type t, bool display)
        {
            return ToList(t, display, null);
        }
        ...
        /// <returns></returns>
        public static IList<EnumInfo> ToList(this Type t, int type)
        {
            return ToList(t, null, type);
        }
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static IList<EnumInfo> ToList(this Type t, bool? display, int? type)
        {
            ...
            if (t.IsEnum)
            {
                ...
                {
                    var einfo = new EnumInfo
                    {
                        Value = value
                    };
                    var cc = Enum.ToObject(t, value);
                    var enumType = cc.GetType();
                    //einfo.Name = Enum.GetName(enumType, cc);
                    einfo.Name = cc.ToString();
                    var fi = enumType.GetField(einfo.Name);
                    ...
                    {
                        einfo.Description = dna.Description;
                        einfo.Display = true;
                        einfo.Sort = sort;
                        if (dna is CustomerDescriptionAttribute dna2)
                        {
                            if ((display.HasValue && dna2.Display != display.Value) 
                                || (type.HasValue && (type.Value&dna2.Type) != dna2.Type)) continue;
                            einfo.Display = dna2.Display;
                            einfo.Type = dna2.Type;
                            einfo.Sort = dna2.Sort;
                            einfo.MutexIds = dna2.MutexIds;
                        }
                    }
                    ret.Add(einfo);
                    sort ++;
            ...
            else
            {
                foreach (var p in t.GetProperties(BindingFlags.Instance | BindingFlags.Public))
                {
                    ...
                    var dna = (DescriptionAttribute) p.GetCustomAttribute(typeof(DescriptionAttribute));
                    if (dna != null && !string.IsNullOrEmpty(dna.Description))
                    {
                        einfo.Description = dna.Description;
                        einfo.Display = true;
            ...
            return ret.OrderBy(obj=>obj.Sort).ToList();
        }
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        public static int[] ToList(this Enum e)
        {
            return (from enumInfo in ToList(e.GetType()) where ((Convert.ToInt32(e) & enumInfo.Value) == enumInfo.Value) select enumInfo.Value).ToArray();
        }

        public static IList<EnumInfo> ToEnumList(this Enum e)
        {
            return (from enumInfo in ToList(e.GetType()) where ((Convert.ToInt32(e) & enumInfo.Value) == enumInfo.Value) select enumInfo).ToList();
        }

        public static T Set<T>(this T se, T e)
        {
            if (!typeof (T).IsEnum) throw new Exception("");
            var el = (from enumInfo in ToList(e.GetType()) where ((Convert.ToInt32(e) & enumInfo.Value) == enumInfo.Value) select enumInfo).ToList();
            var sel = (from enumInfo in ToList(se.GetType()) where ((Convert.ToInt32(se) & enumInfo.Value) == enumInfo.Value) select enumInfo).ToList(); ;
            var ses = sel.Sum(obj => obj.Value);
            foreach (var einfo in el)
            {
                //如果原始值中已存在当前值
                if ((ses & einfo.Value) == einfo.Value) continue;
                if (einfo.MutexIds != null && einfo.MutexIds.Length > 0)
                {
                    foreach (var mutexId in einfo.MutexIds)
                    {
                        foreach (var seinfo in sel.Where(seinfo => seinfo.MutexIds.Contains(mutexId)))
                        {
                            ses &= ~seinfo.Value;
                        }
                    }
            ...
            return (T)Enum.ToObject(typeof(T), ses);
        }
        ...
        public static string ToJsonString(this NameValueCollection value)
        {
            var dic = new Dictionary<string, object>();
            foreach (string key in value.Keys)
            {
                if (string.IsNullOrEmpty(key) || string.IsNullOrEmpty(value[key])) continue;
                if (value[key].Contains(','))
                {
                    dic.Add(key, value[key].Split(','));
                }
                else
                {
                    dic.Add(key, value[key]);
                }
            }
            return ToJsonString(dic);
        }

        public static T ToObject<T>(this string strJson)
        {
            return (T) JsonConvert.DeserializeObject<T>(strJson);
        }
...
/*        public static object ToChangeType(this object value, Type type)
        ...
        {
            return typeof(T).GetProperties(BindingFlags.Instance | BindingFlags.Public).ToDictionary(p => p.Name, p => p.GetValue(value));
        }
        ...
        /// </summary> 
        private static readonly string[] ChineseCharList =
        {
        ...
        public static string ToPinyin(this string value)
        {
            ...
            foreach (var v in value)
            {
                ...
                else if (v >= 19968 && v <= 40869)
                {
                    ...
                    foreach (var strList in ChineseCharList)
                    {
                        if (strList.IndexOf(v) > 0)
                        {
                            c = strList[0];
                            break;
                        }
        ...
        {
            {10, "A"},{11, "B"},{12, "C"},{13, "D"},{14, "E"},{15, "F"},{16, "H"},{17, "J"},{18, "K"},{19, "L"},{20, "M"},
            {21, "N"},{22, "P"},{23, "Q"},{24, "R"},{25, "S"},{26, "T"},{27, "U"},{28, "V"},{29, "W"},{30, "X"},{31, "Y"}
        };
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static string To32String(this int value)
        {
            var ret = "";
            while (true)
            {
                var i = value % 32;
                ret = (i < 10 ? i.ToString(CultureInfo.InvariantCulture) : Numbers[i]) + ret;
                value = value / 32;
                if (value == 0)
                    break;
            }
        ...
        /// 0 I 4 O 6
        /// </summary>
        private static readonly IDictionary<int, string> CouponCodes = new Dictionary<int, string>
        {
            {1, "X"},{2, "8"},{3, "D"},{4, "9"},{5, "R"},{6, "W"},{7, "V"},{8, "2"},{9, "Z"},
            {10, "F"},{11, "M"},{12, "Q"},{13, "3"},{14, "J"},{15, "A"},{16, "H"},{17, "7"},{18, "N"},{19, "L"},{20, "B"},
            {21, "K"},{22, "1"},{23, "C"},{24, "G"},{25, "S"},{26, "T"},{27, "Y"},{28, "E"},{29, "5"},{30, "P"},{31, "U"},
            {32, "0I4O6" }
        };
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static string ToCouponCode(this int value)
        {
            var ret = "";
            while (true)
            {
                ...
                var i = value % 32;
                ret = (i == 0 ? CouponCodes[32][rd.Next(5)].ToString() : CouponCodes[i]) + ret;
                value = value / 32;
                if (value == 0)
                    break;
            }
        ...
        private static readonly DateTime UtcRelativeTime = new DateTime(1970, 1, 1, 0, 0, 0);

        public static DateTime ToDateTime(this double value)
        {
            return UtcRelativeTime.AddSeconds(value).ToLocalTime();
        }

        public static double ToUtc(this DateTime value)
        {
            value = value.ToUniversalTime();
            return value.Subtract(UtcRelativeTime).TotalSeconds;
        }
        ...
        /// <returns></returns>
        public static decimal ToRound(this decimal d, int decimals = 0)
        {
            return Convert.ToDecimal(ToRound(Convert.ToDouble(d), 1, decimals));
        }
        ...
        /// <param name="decimals"></param>
        /// <returns></returns>
        public static double ToRound(this double d, int decimals = 0)
        {
            return ToRound(d, 1, decimals);
        }
        ...
        /// </summary>
        /// <param name="d"></param>
        /// <param name="type">四舍五入法=1,尾数舍弃=2,尾数进位=4</param>
        /// <param name="decimals"></param>
        /// <returns></returns>
        public static decimal ToRound(this decimal d, int type, int decimals)
        {
            return Convert.ToDecimal(ToRound(Convert.ToDouble(d), type, decimals));
        }
        ...
        /// <param name="decimals"></param>
        /// <returns></returns>
        public static double ToRound(this double d, int type, int decimals)
        {
            var dd = 0.5;//四舍五入法
            switch (type)
            {
        ...
        {
            return ToHexString(value, 0, 0, "");
        }

        public static string ToHexString(this byte[] value, string separator)
        {
            return ToHexString(value, 0, 0, separator);
        }

        public static string ToHexString(this byte[] value, bool asc)
        {
            return ToHexString(value, asc, 0, 0, "");
        }

        public static string ToHexString(this byte[] value, bool asc, string separator)
        {
            return ToHexString(value, asc, 0, 0, separator);
        }

        public static string ToHexString(this byte[] value, int offset, int count)
        {
            return ToHexString(value, true, offset, count, "");
        }

        public static string ToHexString(this byte[] value, int offset, int count, string separator)
        {
            return ToHexString(value, true, offset, count, separator);
        }

        public static string ToHexString(this byte[] value, bool asc, int offset, int count)
        {
            return ToHexString(value, asc, offset, count, "");
        }

        public static string ToHexString(this byte[] value, bool asc, int offset, int count, string separator)
        {
            try
            {
                ...
                if (value != null)
                {
                    ...
                    for (var i = offset; i < count; i++)
                    {
                        ...
                        if (asc)
                        {
                            ...
                            returnStr.Append(b.ToString("X2"));
                        }
                        else
                        {
                            if (returnStr.Length > 0)
                                returnStr.Insert(0, separator);
                            returnStr.Insert(0, b.ToString("X2"));
                        }
                    }
                }
        ...
        public static byte[] ForHexString(this string value)
        {
            return ForHexString(value, true, null);
        }

        public static byte[] ForHexString(this string value, bool asc)
        {
            return ForHexString(value, asc, null);
        }

        public static byte[] ForHexString(this string value, bool asc, string separator)
        {
            byte[] ret;
            int index;
            if (string.IsNullOrEmpty(separator))
            {
                ...
                for (var i = 0; i < value.Length / 2; i++)
                {
                    ...
                    if (!asc)
                    {
                        index = value.Length / 2 - 1 - i;
                    }
                    ret[index] = Convert.ToByte(value.Substring(i * 2, 2), 16);
                }
            }
            else
            {
                var vs = value.Split(separator[0]);
                ret = new byte[vs.Length];
                for (var i = 0; i < vs.Length; i++)
                {
                    ...
                    ret[index] = Convert.ToByte(vs[i], 16);
                }
            }
            return ret;
        }

        public static void ForHexString(this string value, int srcOffset, byte[] dst, int dstOffset, int count)
        {
            ForHexString(value, true, null, srcOffset, dst, dstOffset, count);
        }

        public static void ForHexString(this string value, bool asc, string separator, int srcOffset, byte[] dst, int dstOffset, int count)
        {
            int index;
            if (string.IsNullOrEmpty(separator))
            {
                if (value.Length / 2 - srcOffset < count)
                    count = value.Length / 2;
                for (var i = srcOffset; i < count; i++)
                {
                    index = dstOffset + i;
                    if (!asc)
                    {
                        index = dstOffset + value.Length / 2 - 1 - i;
                    }
                    dst[index] = Convert.ToByte(value.Substring(i * 2, 2), 16);
                }
            }
            else
            {
                var vs = value.Split(separator[0]);
                if (vs.Length - srcOffset < count)
                    count = vs.Length;
                for (var i = srcOffset; i < count; i++)
                {
                    index = dstOffset + i;
                    if (!asc)
                    {
                        index = dstOffset + vs.Length - 1 - i;
                    }
                    dst[index] = Convert.ToByte(vs[i], 16);
                }
            }
        }
    }
